...
export const t = derived(messages, ($m) => (key: string, vars?: Record<string, string | number>) =>
...
function resolveLocale(wanted: string | null) {
  ...
  return availableLocales.includes('en-US') ? 'en-US' : availableLocales[0] ?? 'en-US'
}

export async function loadLocale(code: string) {
  const entry = Object.entries(modules).find(([path]) => path.endsWith(`/${code}.json`))
