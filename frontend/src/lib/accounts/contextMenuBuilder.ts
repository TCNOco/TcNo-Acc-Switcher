import type {
  AccountCommands,
  AccountRowProjection,
  SharedMenuItems,
} from "../../components/PlatformAccountAdapter";
...
export interface ContextMenuContext {
  name: string;
  adapter: AccountCommands &
    Pick<
      AccountRowProjection<unknown>,
      | "name"
      | "imageUrl"
      | "manualProfileImage"
      | "tags"
...
export function buildSharedItems(
  ...
  const openPick = () => {
    ...
    ctx.openImagePick(rowId);
  };

  return {
    swapTo: {
      label: savedDataBroken
        ? tr("Security_AccountDataBroken")
        : tr("Context_SwapTo"),
          ...
          action: async () => {
            try {
              const res = await Shortcuts.CreateAccountShortcut(
                name,
                rowId,
                adapter.name(acc) ?? rowId,
                "",
                "",
                adapter.accountLogin(acc) ?? "",
              );
    ...
    forget: {
      ...
      action: async () => {
        const ok = await openConfirm({
          title: tr("Forget"),
          body:
            adapter.forgetPrompt?.(acc) ??
            tr("Prompt_ForgetAccount", { platform: name }),
          style: "yesno",
        });
        if (!ok) return;
        try {
      ...
      action: async () => {
        ...
        const note = await openPrompt({
          ...
          body: tr("Modal_Title_AccountNotes", {
            accountName: adapter.name(acc) ?? rowId,
          }),
          positiveLabel: tr("Ok"),
          negativeLabel: tr("Button_Cancel"),
          initialValue: cur ?? "",
    ...
    gameStats: hasGameStatsSupport
      ? {
          ...
          action: () => ctx.openGameStatsModal(rowId),
        }
      : null,
  };
}
