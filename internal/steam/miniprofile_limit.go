...
// twenty-first request inside a minute - three runs, at three different rates,
// all refusing at exactly twenty-one - and serves 500s for the fifty to sixty
// seconds that follow.
//
// For contrast, the profile XML on the same hostname cleared 240 requests at 26 a
// second without complaint, and the avatar CDN never refused at all. So this leash
...
const (
	...
	// refused. Retrying inside the block was measured to neither shorten nor
	// lengthen it, so there is nothing to gain by probing a way back in - it waits
	// the window out once, with a few seconds to spare.
...
// sustained-rate rule - the one an inventory scraper would follow - would turn a
// four second refresh into forty-eight and buy nothing. What must not happen is a
// second burst inside the same minute, and that is exactly what this defers.
...
// has had enough - not 429, which is what a reader would expect and what an
// earlier version of this waited for. 429 is here too because it is the harsher
// tier, reported to last hours rather than a minute.
