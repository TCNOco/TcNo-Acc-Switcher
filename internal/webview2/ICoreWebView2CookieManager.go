...
// GetCookies requests every cookie matching uri.
//
// It is asynchronous. The IDL declares
// GetCookies([in] LPCWSTR uri, [in] ICoreWebView2GetCookiesCompletedHandler*),
// so handler receives the result and the caller must be pumping messages until it
// fires. The upstream binding passed an out-parameter in the handler's place,
// which the runtime dereferences as an interface pointer and faults on.
