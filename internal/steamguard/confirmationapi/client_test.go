...
func TestOnlyADisabledRedirectReadsAsReauth(t *testing.T) {
	reauth := classifyProtocolError(&protocol.Error{
		Code: protocol.CodeRedirectDenied, State: protocol.StateDenied,
		Detail: protocol.DetailRedirectDisabled,
	})
	assertFailure(t, reauth, FailureReauth)

	for _, detail := range []string{protocol.DetailRedirectLimit, "redirect_host", "redirect_method", ""} {
		failure := assertFailure(t, classifyProtocolError(&protocol.Error{
			Code: protocol.CodeRedirectDenied, State: protocol.StateDenied, Detail: detail,
		}), FailureFailed)
		if failure.Detail != detail {
			t.Fatalf("detail = %q, want %q", failure.Detail, detail)
		}
	}
}
