...
void download_install_missing_runtimes()
{
	...
	if (!min_webview_met || !min_aspcore_met || !min_desktop_runtime_met || !min_vc_met || test_mode)
	{
		...
		if (!min_desktop_runtime_met || test_downloads)
		{
			...
			{
				cout << "Failed to download and install .NET 6 Desktop Runtime. Please download it here:" << endl <<
		...
		if (!min_aspcore_met || test_downloads)
		{
			...
			{
				cout << "Failed to download and install ASP.NET Core 6.0 Runtime. To download: 1. Click the link below:" << endl <<
	...
	cout << " - Windows Desktop Runtime 6.0.0+" << endl <<
		" - ASP.NET Core 6.0.0+." << endl <<
