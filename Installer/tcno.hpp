...
void download_install_missing_runtimes()
{
	...
	if (!min_webview_met || !min_aspcore_met || !min_desktop_runtime_met || !min_vc_met || test_mode)
	{
		...
		if (!min_aspcore_met) {
			cout << " + Microsoft Microsoft ASP.NET Core 6.0.1 Runtime [~7,90 MB]" << endl;
			total += 8;
		}
		if (!min_desktop_runtime_met) {
			cout << " + Microsoft .NET 6.0.1 Desktop Runtime [~52,3 MB]" << endl;
			total += 52;
		}
		...
		if (!min_desktop_runtime_met || test_downloads)
		{
			...
			{
				cout << "Failed to download and install .NET 6.0.1 Desktop Runtime. Please download it here:" << endl <<
		...
		if (!min_aspcore_met || test_downloads)
		{
			...
			{
				cout << "Failed to download and install ASP.NET Core 6.0.1 Runtime. To download: 1. Click the link below:" << endl <<
					"https://dotnet.microsoft.com/download/dotnet/6.0.1/runtime" << endl <<
	...
	cout << " - Windows Desktop Runtime 6.0.1+" << endl <<
		" - ASP.NET Core 6.0.1+." << endl <<
